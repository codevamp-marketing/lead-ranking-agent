"""
agent/lead_ranking_agent.py — Lead Ranking Agent v3.0
=======================================================
FIXES v3.0
──────────
FIX 1 — Removed broken empty try/except block that caused IndentationError.
  The sed command in terminal session accidentally deleted the body of the
  try block, leaving `try:` with no body. Fixed by removing the entire
  broken block cleanly.

FIX 2 — _get_supabase() now reads os.environ directly after load_dotenv(),
  same pattern as conversation_store.py and rag_engine.py.

FIX 3 — All Supabase operations properly use asyncio.to_thread() so the
  blocking psycopg2 LISTEN loop doesn't starve async tasks.
"""
...
def _get_supabase():
    ...
    if _supabase_client is None:
        ...
        logger.info("lead_agent_supabase_ok")
...
_processing_leads: set[str] = set()


async def _remove_from_processing(lead_id: str, delay_seconds: int = 300) -> None:
    await asyncio.sleep(delay_seconds)
    _processing_leads.discard(lead_id)
    logger.debug("idempotency_lock_released", lead_id=lead_id)


async def _log_ai_activity(lead_id: str, score: int, lead_type: str,
                            action: str, ltv: float) -> None:
    description = (
        ...
        try:
            async with httpx.AsyncClient(timeout=settings.CRM_TIMEOUT) as http:
                ...
                if resp.status_code == 200:
                    return True
                logger.warning("crm_patch_bad_status",
                               lead_id=lead_id, attempt=attempt, status=resp.status_code)
    ...
    while True:
        ...
        for entry in batch:
            ...
            try:
                db = _get_supabase()
                await asyncio.to_thread(
                    lambda e=entry, d=db: d.table("Lead").update(e.payload).eq("id", e.lead_id).execute()
                )
                logger.info("dlq_reprocess_ok", lead_id=entry.lead_id, attempts=entry.attempts)
            except Exception as exc:
                logger.error("dlq_reprocess_failed",
                             lead_id=entry.lead_id, attempts=entry.attempts, error=str(exc))
                if entry.attempts < 10:
                    _dlq.append(entry)
                else:
                    logger.critical("dlq_entry_abandoned",
                                    lead_id=entry.lead_id, payload=entry.payload)
...
async def process_lead(lead: dict) -> None:
    ...
    if not lead_id:
        ...
        return
    if lead_id in _processing_leads:
        logger.warning("duplicate_notify_ignored", lead_id=lead_id)
        return
    _processing_leads.add(lead_id)
    asyncio.create_task(
        _remove_from_processing(lead_id, delay_seconds=300),
        name=f"idempotency-cleanup-{lead_id}"
    )
    ...
    try:
        ...
        logger.error("process_lead_failed",
                     lead_id=lead_id, name=name, error=str(exc), exc_info=True)
...
def _task_error_handler(task: asyncio.Task) -> None:
    ...
    exc = task.exception()
    if exc:
        logger.error("unhandled_task_exception",
        ...
        try:
            ...
            while conn.notifies:
                ...
                try:
                    ...
                    logger.error("payload_parse_error",
                                 error=str(exc), raw=notify.payload[:200])
                    continue
                task = asyncio.create_task(
                    process_lead(lead),
                    name=f"lead-{lead.get('id', 'unknown')}"
                )
...
async def main() -> None:
        ...
        async with httpx.AsyncClient(timeout=10) as http:
            try:
                ...
                if r.status_code < 500:
                    logger.info("crm_reachable", status=r.status_code)
                else:
    ...
    try:
        db = _get_supabase()
        await asyncio.to_thread(
            lambda: db.table("Lead").select("id").limit(1).execute()
        )
